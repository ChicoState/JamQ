...
let SSHSetupCommand = class SSHSetupCommand extends base_1.SSHBaseCommand {
    ...
    run(inputs, options) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            const { getConfigPath } = yield Promise.resolve().then(() => require('@ionic/cli-utils/lib/ssh-config'));
            ...
            if (setupChoice === CHOICE_AUTOMATIC) {
                ...
                if (!pubkeyExists && !keyExists) {
                    this.env.log.info('The automatic SSH setup will do the following:\n' +
                        ...
                        `3) Modify your SSH config (${chalk_1.default.bold(prettyPath(sshconfigPath))}) to use the generated SSH private key for our server(s).`);
                ...
                if (pubkeyExists && keyExists) {
                    this.env.log.info(`Using your previously generated key: ${chalk_1.default.bold(prettyPath(keyPath))}.\n` +
                        `You can generate a new one by deleting it.`);
                }
                else {
                    yield this.env.runCommand(['ssh', 'generate', keyPath]);
                }
                yield this.env.runCommand(['ssh', 'add', pubkeyPath, '--use']);
            }
            else if (setupChoice === CHOICE_MANUAL) {
                yield this.env.runCommand(['ssh', 'add']);
            }
