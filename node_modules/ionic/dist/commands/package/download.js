...
let PackageDownloadCommand = class PackageDownloadCommand extends command_1.Command {
    run(inputs, options) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            if (buildId) {
                this.env.tasks.next(`Retrieving information about build ${chalk_1.default.bold(String(buildId))}`);
            }
            else {
                ...
                if (latestBuilds.length === 0) {
                    this.env.tasks.end();
                    return this.env.log.warn(`You don't have any builds yet! Run ${chalk_1.default.green('ionic package build --help')} to learn how.`);
                }
            ...
            const downloadTask = this.env.tasks.next(`Downloading build ${chalk_1.default.bold(String(build.id))}`);
            yield pkg.downloadBuild(build, ws, { progress: (loaded, total) => {
                    downloadTask.progress(loaded, total);
                } });
            this.env.tasks.end();
            this.env.log.ok(`Build ${chalk_1.default.bold(String(build.id))} downloaded as ${chalk_1.default.bold(prettyPath(buildFilename))}.`);
        });
    }
};
